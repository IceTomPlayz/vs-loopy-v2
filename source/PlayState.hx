...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'building': //gloopy/macaroni
				var building:BGSprite = new BGSprite('gloopy/stages-normal/building', -600, -200, 0.9, 0.9);
				add(building);

			case 'tractor': //ball-deodorant
				var tractor:BGSprite = new BGSprite('gloopy/stages-mad/tractor', -600, -200, 0.9, 0.9);
				add(tractor);

			case 'street': //improvise/geek
				var street:BGSprite = new BGSprite('bbg/stages-normal/street', -600, -200, 0.9, 0.9);
				add(street);

			case 'sunset': //improvise/geek
				var sunset:BGSprite = new BGSprite('bbg/stages-sunset/street-sunset', -600, -200, 0.9, 0.9);
				add(sunset);
		...
		if(curSong.toLowerCase() == "gloopy") {
			timeBar.createFilledBar(0xFF000000, 0xFF74D948);
		}

		if(curSong.toLowerCase() == "macaroni") {
			timeBar.createFilledBar(0xFF000000, 0xFF74D948);
		}

		if(curSong.toLowerCase() == "ball-deodorant") {
			timeBar.createFilledBar(0xFF000000, 0xFF74D948);
		}

		if(curSong.toLowerCase() == "election") {
			timeBar.createFilledBar(0xFF000000, 0xFFDFAA65);
		}

		if(curSong.toLowerCase() == "blunder") {
			timeBar.createFilledBar(0xFF000000, 0xFFFA0255);
		}

		if(curSong.toLowerCase() == "gloopy-erect") {
			timeBar.createFilledBar(0xFF000000, 0xFFB23891);
		}

		if(curSong.toLowerCase() == "macaroni-erect") {
			timeBar.createFilledBar(0xFF000000, 0xFFB23891);
		}

		if(curSong.toLowerCase() == "ball-deodorant-erect") {
			timeBar.createFilledBar(0xFF000000, 0xFFB23891);
		}
	...
	public function updateScore(miss:Bool = false)
	{
		...
		{
			if(ratingName == '?') {
				scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' / 10' + ' | Accuracy: ' + ratingName;
			} else {
				scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' / 10' + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ' [' + ratingFC + '] ';
			}
		}
		else
		{
	...
	override public function update(elapsed:Float)
	{
		...
		if(curSong == 'blunder' && songMisses > 10)
		{
